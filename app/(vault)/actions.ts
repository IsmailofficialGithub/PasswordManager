...
/**
 * Get all environment credentials related to a specific project (by title)
 */
export async function getProjectEnvCredentials(
  title: string
): Promise<{ success: boolean; credentials?: CredentialWithTags[]; error?: string }> {
  try {
    const { user, error: authError } = await requireAuthAndUnlock();
    ...
    const credentials: CredentialWithTags[] = data.map((d: any) => ({
      ...d,
      tags: d.vault_credential_tags?.map((ct: any) => ct.vault_tags) || [],
    }));
